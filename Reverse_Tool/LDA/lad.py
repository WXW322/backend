...
from protocol_analysis.words_deal import message_dealer
...
def getKeyWords(dirPath):
    messages = read_datas(dirPath)
    for message in messages:
        print(str(message))
    wordList, _ = get_messages(messages, 3, 5, 8)
    for wList in wordList:
        wMerger = WordsMerger(wList)
        print('before')
        print(wList)
        print(wMerger.mergeWords(wList))
        print('after')


if __name__ == '__main__':
    #getKeyWords('/home/wxw/data/httpDatas/http_test')
    messages = read_datas('/home/wxw/data/httpDatas/http_test')
    print(get_messages(messages, 3, 5, 5))
